...
def randomForest(input_path):
    ...
    xgb_model = RandomForestClassifier(n_estimators=100,  # Number of trees in the forest
    max_depth=5,       # Maximum depth of each tree
    random_state=42) 
    xgb_model.fit(x_train, y_train)
    y_pred = xgb_model.predict(x_test)
    y_pr = xgb_model.predict(x_train)
    acc = accuracy_score(y_train, y_pr)
    accuracy = accuracy_score(y_test, y_pred)
    print(acc)
    print(accuracy)
#randomForest('./Data/preprocessed/final_fraud.csv')
...
def xgb(input_path):
    ...
    print(acc)
    print(accuracy)
xgb('./Data/preprocessed/final_fraud.csv')
 """